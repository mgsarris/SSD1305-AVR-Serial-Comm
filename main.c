...
/********* This was adafruits splash screen, but I kinda erased it *********/
// static uint8_t buffer[32 * 128 / 8] = {
// 0x01, 0x01, 0x7F, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void USART_Receive(void)
{
    ...
    while (i < CHAR_PER_LINE)
    {
        ...
        if (input.inputData[i] == 8)
        {
            continue;
        }
        i++;
    }
    // return *dataPoint = inputData[0];
}
...
int main(void)
{
    ...
    // i2c_write(0x40);
    //     for (x=0; x<16; x++) {
    //         // i2c_write(GLCDFONTDECL[i]);
    //         for (j=0;j<5;j++){
    //             i2c_write(GLCDFONTDECL[i][j]);
    //         }
    //         i++;
    //     }
    // i--;
    ...
    ssd1306_command(0);
    ssd1306_command(127);
    for (i = 0; i<(sizeof(test2)/sizeof(char)-1);i++)
    {
        ssd1306_start();
        i2c_write(0x40);
        for(x=0;x<5;x++)
        {
            i2c_write(GLCDFONTDECL[test2[i]-0x20][x]);
        }
        i2c_stop();
    }
